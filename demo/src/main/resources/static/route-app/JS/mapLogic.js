...
function bindMapEventListeners() {
    ...
    // 4. Initialize Autocomplete (UC7-FR1-FR6)
    const autocompleteStart = new google.maps.places.Autocomplete(startInput);
    const autocompleteEnd = new google.maps.places.Autocomplete(destInput);
    autocompleteStart.bindTo('bounds', map);
    autocompleteEnd.bindTo('bounds', map);

    // Auto-focus the map on entry for selected places
    ...
    // --- PRINT DIRECTIONS LOGIC ---
    const printLinks = document.querySelectorAll('.ar-print-link');
    printLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            e.preventDefault();
            const directionsContainer = document.getElementById('ar-directions-container');
            if (directionsContainer) {
                // Open a new blank window for printing
                const printWindow = window.open('', '_blank', 'width=800,height=600');
                if (!printWindow) {
                    alert('Please allow pop-ups for this site to print directions.');
                    return;
                }
                
                // Write a clean HTML structure with specific print styling
                printWindow.document.write(`
                    ...
                    <head>
                        ...
                        <style>
                            ...
                            /* Margin & Spacing Optimization for 1 Page */
                            @page { margin: 0.5cm; }
                            
                            /* Minimal structural CSS to match original page design roughly */
                            .dir-city-header, .dir-city-footer {
                                font-weight: bold;
                                font-size: 1.2rem;
                                margin-top: 15px;
                                margin-bottom: 10px;
                            ...
                            .dir-step {
                                ...
                                margin-bottom: 8px; /* Tighter layout */
    ...
    // --- SAVE ROUTE LOGIC (Backend Integration) ---
    const btnSaveFav = document.getElementById('btn-ar-save-fav');
    if (btnSaveFav) {
        btnSaveFav.addEventListener('click', async (e) => {
            ...
            if (!userID) {
                ...
                return; // Might implement redirect later
            }

            const currentDirs = directionsRenderer.getDirections();
            if (!currentDirs || !currentDirs.routes || currentDirs.routes.length === 0) {
                alert("No active route to save.");
                return;
            }

            const primaryRoute = currentDirs.routes[0];
            const polyline = primaryRoute.overview_polyline;
            
            // Build Stops Payload
            const stops = [];
            const legs = primaryRoute.legs;
            
            // First leg start = Origin
            stops.push({
                stopType: "ORIGIN",
                placeId: "origin_marker", // Google maps doesn't easily expose this back out of the direction result without geocoding, mock for now
                addressString: legs[0].start_address,
            ...
            for (let i = 0; i < legs.length - 1; i++) {
                stops.push({
                    stopType: "WAYPOINT",
                    placeId: "wp_marker_" + i,
                    addressString: legs[i].end_address,
                    latitude: legs[i].end_location.lat(),
                    longitude: legs[i].end_location.lng()
                });
            }

            // Final leg end = Destination
            const finalLeg = legs[legs.length - 1];
            stops.push({
                stopType: "DESTINATION",
                placeId: "dest_marker",
                addressString: finalLeg.end_address,
                latitude: finalLeg.end_location.lat(),
                longitude: finalLeg.end_location.lng()
            });

            // Default Name: "Start to Destination"
            const arStartDisplay = document.getElementById('ar-start-display');
            const arEndDisplay = document.getElementById('ar-end-display');
            const originCity = arStartDisplay ? arStartDisplay.value.trim() : "Start";
            const destCity = arEndDisplay ? arEndDisplay.value.trim() : "Destination";
            const defaultName = `${originCity} to ${destCity}`;
            ...
            try {
                ...
                if (response.ok) {
                    alert("Route successfully saved as: " + data.name);
                } else {
